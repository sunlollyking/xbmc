...
void CGameClient::NotifyError(GAME_ERROR error)
{
  ...
  if (Streams().HardwareRenderingAttempted())
  {
    ...
    if (!wanted.empty() && !available.empty())
    {
      ...
      MESSAGING::HELPERS::ShowOKDialogText(
          CVariant{35210},
          CVariant{StringUtils::Format(CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(35298), wanted, available)});
    }
    else if (!wanted.empty())
    {
      ...
      MESSAGING::HELPERS::ShowOKDialogText(
          CVariant{35210}, CVariant{StringUtils::Format(CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(35299), wanted)});
    }
    else
    {
      // Failed to play game
