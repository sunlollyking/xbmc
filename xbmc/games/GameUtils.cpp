                                             ...
                                             const GameClientVector& candidates)
{
  ...
  CGameClientDatabase db;
  if (!db.Open())
    return "";

  const std::string gameClient = db.GetGameClientForGame(path);
  if (gameClient.empty())
    return "";
...
bool CGameUtils::ChooseAndSetDefaultGameClient(const CFileItem& item)
{
  ...
  const std::string currentGameClient = db.GetGameClient(path);
  ...
  if (!db.SetGameClient(path, gameClient))
    return false;

  if (gameClient.empty())
    CLog::Log(LOGDEBUG, "GAME: Forgot the emulator for {}", CURL::GetRedacted(path));
  else
    CLog::Log(LOGDEBUG, "GAME: Remembered emulator {} for {}", gameClient,
