...
///                  _boolean_,
///     @return **True** if cheats were found for the currently-playing game\,
///     **False** otherwise.
///     <p><hr>
...
/// -----------------------------------------------------------------------------
// clang-format off
constexpr std::array<InfoMap, 245> listitem_labels = {{
...
CGUIInfo CGUIInfoManager::GetMultiInfo(int id) const
{
  ...
  // a skin with more multi-info blocks than the range allows gets logged and
  // then indexes past the end, so the bound is worth keeping here too
