...
void CInputManager::ProcessCec()
{
  ...
  for (auto* handler : m_cecInputHandlers)
  {
    CKey key;
    if (handler->GetCecKey(key))
      QueueCecKey(key);
  }
}
...
void CInputManager::RegisterCecInputHandler(CEC::ICecInputHandler* handler)
{
  std::unique_lock lock(m_cecHandlingMutex);

  if (std::ranges::find(m_cecInputHandlers, handler) == m_cecInputHandlers.end())
    m_cecInputHandlers.insert(m_cecInputHandlers.begin(), handler);
}

void CInputManager::UnregisterCecInputHandler(CEC::ICecInputHandler* handler)
{
  std::unique_lock lock(m_cecHandlingMutex);

  const auto it = std::ranges::find(m_cecInputHandlers, handler);
  if (it != m_cecInputHandlers.end())
    m_cecInputHandlers.erase(it);
}
