...
bool CRPRendererFBO::CopyFrameForShaders(CRenderBufferFBO* renderBuffer)
{
  ...
  // Kodi is drawing the GUI around the game and scissors as it goes, in screen
  // coordinates that mean nothing to this texture
...
void CRPRendererFBO::DrawBlackBars()
{
  ...
  // the default object and needs none of this. Created once and reused.
  if (m_vao == 0)
...
void CRPRendererFBO::Render(uint8_t alpha)
{
  ...
  // The chain reads a texture holding just the client's frame and writes one
  // the size of the destination. Any failure turns the preset off and leaves
  // the direct path below to draw the frame, so a filter that cannot run costs
  // the picture nothing.
  ...
  {
    // Sized to what is actually being drawn rather than m_fullDestWidth/Height,
    // which describe the picture at fullscreen. A game drawn into a GUI control
    // would have its filter rendered full size and scaled down, which anything
    // working at the pixel level does not survive.
...
#if !defined(HAS_GLES)
  // As in DrawBlackBars: a core profile has no default vertex array object.
  if (m_vao == 0)
    glGenVertexArrays(1, &m_vao);
  glBindVertexArray(m_vao);
#endif
