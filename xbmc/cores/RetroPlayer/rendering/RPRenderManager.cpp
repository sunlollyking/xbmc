...
void CRPRenderManager::DestroyContext()
{
  ...
  // thread stopped the player, where deleting them is undefined.
...
bool CRPRenderManager::Create(unsigned int width, unsigned int height)
{
  ...
  CLog::Log(LOGERROR, "RetroPlayer[RENDER]: No buffer pool could provide a {}x{} framebuffer",
            width, height);
...
uintptr_t CRPRenderManager::GetCurrentFramebuffer(unsigned int width, unsigned int height)
{
  ...
  // loses that frame; one that caches the answer draws the whole frame into
  // nothing.
  ...
  {
    CLog::Log(LOGDEBUG,
              "RetroPlayer[RENDER]: Client wants {}x{}, larger than the {}x{} framebuffer", width,
              height, m_hwBufferWidth, m_hwBufferHeight);
  ...
  // Clients cache this and render against it for the rest of the session.
...
void CRPRenderManager::RenderFrame(unsigned int width, unsigned int height)
{
  ...
  // A pool that cannot spare a buffer publishes the client's own, which is
