                                    ...
                                    CGUIInfoManager* guiInfoManager)
{
  ...
  if (m_ros2)
  {
    ...
    {
      m_guiInfo =
          std::make_unique<CSmartHomeGuiInfo>(*guiInfoManager, *m_ros2->GetSystemHealthHUD(),
                                              *m_ros2->GetVehicleHUD(), *m_ros2->GetStationHUD());
...
void CSmartHomeServices::Deinitialize()
{
  ...
  CLog::Log(LOGDEBUG, "SMARTHOME: Services deinitialized");
}
