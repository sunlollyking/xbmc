...
void CRos2InputPublisher::Initialize()
{
  ...
  m_peripheralPublisher =
      m_node->create_publisher<oasis_msgs::msg::PeripheralScan>(publishPeripheralsTopic, 1);
  m_inputPublisher =
      m_node->create_publisher<oasis_msgs::msg::PeripheralInput>(publishInputTopic, 1);
                                         ...
                                         std::vector<PeripheralInfo>& peripheralScan)
{
  for (const PERIPHERALS::PeripheralPtr& peripheral : peripherals)
  {
    ...
    infoMessage.capturing_input = m_inputManager.IsCapturingInput(peripheral->Location());
